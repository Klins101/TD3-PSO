...
def plot_training_curves(rewards, lengths, save_dir):
    ...
    # Plot rewards
    axes[0].plot(rewards, alpha=0.3, color='green', label='Episode Reward')
    axes[0].plot(episodes_smooth, rewards_smooth, color='green', linewidth=2, label='Smoothed (50ep)')
    axes[0].set_xlabel('Episode')
    axes[0].set_ylabel('Reward')
    axes[0].set_title('TD3-PSO Training - Episode Rewards')
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)

    # Plot episode lengths
    axes[1].plot(lengths, color='orange', alpha=0.7)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'training_curves.png'), dpi=350)
...
def train():
    ...
    print_training_header(config, algorithm='TD3-PSO')
    save_dir = create_directories(config['save_dir_td3pso'])
    env = CrazyflieEnv(config)
    agent = TD3_PSO(config, device)
    episode_rewards = []
    episode_lengths = []
    best_reward = -float('inf')
    log_file = init_training_log(save_dir, algorithm='td3pso')
    ...
    for episode in range(config['max_episodes']):
        ...
        if (episode + 1) % config['pso_freq'] == 0 and len(agent.replay_buffer) >= config['warmup_steps']:
            agent.apply_pso(env)
